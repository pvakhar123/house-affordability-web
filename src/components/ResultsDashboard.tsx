...
export default function ResultsDashboard({ report, onReset, summaryLoading, userLocation, traceId, onLoadReport }: Props) {
  ...
  const navItems: { key: string; label: string }[] = [
    ...
    // Is it a good time? Is this the right place? What's available?
    { key: "market", label: "Market" },
    ...(report.neighborhoodInfo || coordinates || displayLocation ? [{ key: "neighborhood", label: "Area" }] : []),
    ...(userLocation ? [{ key: "properties", label: "Properties" }] : []),
    // Should I buy? How do I finance? Upfront costs?
    ...
    // What could go wrong? Long-term outlook? Deep dive
    { key: "risk", label: "Risk" },
