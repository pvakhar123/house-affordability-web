 ...
 *
 * In RAG, you need a collection of documents the AI can search through.
 * These could come from:
 *   - PDFs, web pages, databases, APIs
 *   - Internal docs, FAQs, manuals
 *
 * The key insight: LLMs have a knowledge cutoff and can hallucinate.
 * By retrieving REAL documents and injecting them into the prompt,
 * the LLM answers grounded in facts rather than guessing.
 *
 * Here we hardcode a small knowledge base for learning.
 * In production, you'd load these from a database or file system.
 */
...
export interface Document {
  ...
  title: string;
  content: string;
  source: string; // where the info came from (for citations)
  category: string;
}
 ...
 * STEP 2: CHUNKING
 *
 * Real documents are often too long to fit in an LLM's context window.
 * "Chunking" = splitting documents into smaller, meaningful pieces.
 *
 * Common strategies:
 *   - Fixed size (every 500 chars) — simple but can split mid-sentence
 *   - Sentence-based — split on periods/newlines
 *   - Semantic — split on topic changes (most sophisticated)
 *   - Overlapping — each chunk includes some text from the previous one
 *
 * Our documents below are already chunk-sized for simplicity.
 * In production, you'd use a chunking library like langchain's text splitter.
 */
